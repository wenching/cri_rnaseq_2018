...
Contact     : Wen-Ching Chan <wchan10@bsd.uchicago.edu>
  '''.format(SELF_FILE, VERSION),
     epilog='''
%(prog)s Version {}
--projdir /Users/wenching/Desktop/Sync/CRI/CRI-Pipeline/CRI_RNAseq_2018/example/DLBC
--metadata /Users/wenching/Desktop/Sync/CRI/CRI-Pipeline/CRI_RNAseq_2018/example/DLBC.metadata.txt
--config /Users/wenching/Desktop/Sync/CRI/CRI-Pipeline/CRI_RNAseq_2018/example/DLBC.pipeline.yaml
--systype cluster
--log_file /Users/wenching/Desktop/Sync/CRI/CRI-Pipeline/CRI_RNAseq_2018/Build_RNAseq.DLBC.YYYY-MM-DD_HH_MM_SS.log
    '''.format(VERSION)
    )
    ...
    essential_argument.add_argument(
        '-m', '--metadata', dest='meta_data_path', 
    ...
    logging.info("METADATA:        = [ {} ]".format(args.meta_data_path))
    ...
    args.job_script = "{}/Submit_RNAseq.{}.sh".format(args.proj_dir, args.proj)
    args.submitter = "{}/Submit_RNAseq.{}.bds".format(args.proj_dir, args.proj)
    ...
    if not os.path.isfile(args.meta_data_path):
        sys.exit("cannot find {}".format(args.meta_data_path))
    else:
        curr_meta_data_md5 = util.hashlib.md5(args.meta_data_path)    
    
    # Build_RNAseq.pl > BuildProject.pm > ParseMetadata.pm > SampleList.pm
    sample_list = pandas.read_csv(args.meta_data_path, sep="\t", comment='#')
    logging.debug(type(sample_list.index)) # pandas.core.frame.DataFrame
    logging.debug(list(map(type, list(sample_list))))
    logging.debug(sample_list['Sample'])
    
    logging.debug("ROW_NAMES: [ {} ]".format(' | '.join(str(v) for v in sample_list.index)))
    logging.debug("COL_NAMES: [ {} ]".format(' | '.join(sample_list.columns)))
    logging.debug(sample_list[['Sample', 'ReadGroup']])
    logging.debug(sample_list.loc[:, ['Sample', 'ReadGroup']]) # ~ sample_list.loc[sample_list.index, ['Sample', 'ReadGroup']]
    logging.debug(sample_list.loc[0:2, ['Sample', 'ReadGroup']])
    
    
    logging.info("VERIFIY of Sample Information\n")
    
    genome_list = sample_list['Genome'].drop_duplicates().values.tolist()
    ...
    df_lib_type_count_per_lib = \
        sample_list.loc[:,['Library', 'LibType']].drop_duplicates().groupby(by = "Library").agg({'LibType': 'count'})
    ...
    df_Flavor_count_per_lib = \
        sample_list.loc[:,['Library', 'Flavor']].drop_duplicates().groupby(by = "Library").agg({'Flavor': 'count'})
    ...
    project_cfg['project']['meta_data_path'] = args.meta_data_path
